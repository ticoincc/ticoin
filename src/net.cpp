...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (TICO users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "TICO " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TICO Core is probably already running."), addrBind.ToString());
        else
