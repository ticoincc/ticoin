...
#include "ui_zticoincontroldialog.h"
...
std::list<std::string> ZTicoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTicoinControlDialog::listMints;

ZTicoinControlDialog::ZTicoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTicoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTicoinControlDialog::~ZTicoinControlDialog()
{
    delete ui;
}

void ZTicoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTicoinControlDialog::updateList()
{
...
void ZTicoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTicoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTicoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTicoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTicoinControlDialog::GetSelectedMints()
{
...
void ZTicoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
