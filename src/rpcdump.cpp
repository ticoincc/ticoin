...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TICO address or script");
    }
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by TICO %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
Value bip38encrypt(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"ticoinaddress\"\n"
            "\nEncrypts a private key corresponding to 'ticoinaddress'.\n"
            "\nArguments:\n"
            "1. \"ticoinaddress\"   (string, required) The ticoin address for the private key (you must hold the key already)\n"
