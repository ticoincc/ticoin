...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zTicoin spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zTicoin serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TICOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TICOMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running TICOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
